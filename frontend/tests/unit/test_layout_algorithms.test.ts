...
import { DocumentCaddyId } from "@/features/workspace/domain/value-objects/identifiers";
...
describe("Layout Algorithm Unit Tests", () => {
  ...
  describe("StackedLayoutStrategy", () => {
    ...
    it("should place active document at front position", () => {
      const results = stackedLayout.calculateLayout(
        mockDocuments,
        workspaceSize
      );

      const activeDoc = results.find(
        (r) => r.id.toString() === mockDocuments[0].id.toString()
      );
    ...
    it("should maintain document dimensions in stacked mode", () => {
      ...
      results.forEach((result) => {
        const originalDoc = mockDocuments.find(
          (d) => d.id.toString() === result.id.toString()
        );
        expect(result.dimensions.getWidth()).toBe(
          originalDoc!.currentDimensions.getWidth()
        );
        expect(result.dimensions.getHeight()).toBe(
          originalDoc!.currentDimensions.getHeight()
        );
      });
    });
  ...
  describe("GridLayoutStrategy", () => {
    ...
    it("should arrange documents in a grid pattern", () => {
      ...
      const uniqueXPositions = [...new Set(positions.map((p) => p.x))].sort(
        (a, b) => a - b
      );
      const uniqueYPositions = [...new Set(positions.map((p) => p.y))].sort(
        (a, b) => a - b
      );
    ...
    it("should calculate appropriate grid dimensions for different document counts", () => {
      ...
      let results = gridLayout.calculateLayout(
        [mockDocuments[0]],
        workspaceSize
      );
      expect(results).toHaveLength(1);
      ...
      const fourDocs = [
        ...mockDocuments,
        { ...mockDocuments[0], id: DocumentCaddyId.create() },
      ];
      ...
      results.forEach((result) => {
        expect(
          result.position.getX() + result.dimensions.getWidth()
        ).toBeLessThanOrEqual(workspaceSize.getWidth());
        expect(
          result.position.getY() + result.dimensions.getHeight()
        ).toBeLessThanOrEqual(workspaceSize.getHeight());
      });
    });
    ...
    it("should maintain equal cell sizes for all documents", () => {
      ...
      results.forEach((result) => {
        expect(result.dimensions.getWidth()).toBe(
          firstDocDimensions.getWidth()
        );
        expect(result.dimensions.getHeight()).toBe(
          firstDocDimensions.getHeight()
        );
      });
    });
    ...
    it("should respect minimum dimensions", () => {
      ...
      results.forEach((result) => {
        expect(result.dimensions.getWidth()).toBeGreaterThanOrEqual(
          Dimensions.minimum().getWidth()
        );
        expect(result.dimensions.getHeight()).toBeGreaterThanOrEqual(
          Dimensions.minimum().getHeight()
        );
      });
    });
    ...
    it("should handle large numbers of documents efficiently", () => {
      ...
      expect(
        Math.abs(uniqueXPositions.length - uniqueYPositions.length)
  ...
  describe("FreeformLayoutStrategy", () => {
    ...
    it("should preserve original document positions", () => {
      const results = freeformLayout.calculateLayout(
        mockDocuments,
        workspaceSize
      );
    ...
    it("should maintain z-index based on active state", () => {
      ...
      const inactiveDocs = results.filter(
        (r) => r.id.toString() !== mockDocuments[0].id.toString()
      );

      expect(activeDoc!.zIndex).toBeGreaterThan(
        Math.max(...inactiveDocs.map((d) => d.zIndex))
      );
    });
  ...
  describe("Layout Algorithm Performance", () => {
    it("should handle large document sets efficiently", () => {
      const largeDocumentSet = Array.from({ length: 100 }, (_, i) => ({
        ...
        currentDimensions: Dimensions.fromValues(
          200 + (i % 100),
          150 + (i % 80)
        ),
        isActive: i === 0,
      ...
      strategies.forEach((strategy) => {
        ...
        const results = strategy.calculateLayout(
          largeDocumentSet,
          workspaceSize
        );
