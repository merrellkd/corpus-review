...
import { Workspace } from '../../src/domains/workspace/domain/aggregates/workspace';
import { DocumentCaddy } from '../../src/domains/workspace/domain/entities/document-caddy';
import { WorkspaceId, DocumentCaddyId } from '../../src/domains/workspace/domain/value-objects/identifiers';
import { Position, Dimensions } from '../../src/domains/workspace/domain/value-objects/geometry';
import { LayoutModeType, LayoutMode } from '../../src/domains/workspace/domain/value-objects/layout-mode';
...
describe('Workspace Domain Logic Unit Tests', () => {
  ...
  beforeEach(() => {
    ...
    workspace = Workspace.create('Test Workspace', layoutMode, workspaceDimensions);
  });
  ...
  describe('Workspace Creation', () => {
    ...
    it('should generate unique workspace ID', () => {
      const workspace1 = Workspace.create('Workspace 1', layoutMode, workspaceDimensions);
      const workspace2 = Workspace.create('Workspace 2', layoutMode, workspaceDimensions);

      expect(workspace1.getId().toString()).not.toBe(workspace2.getId().toString());
    });
    ...
    it('should set creation and modification timestamps', () => {
      ...
      expect(lastModified.getTime()).toBeGreaterThanOrEqual(createdAt.getTime());
    });
  });
  ...
  describe('Document Management', () => {
    ...
    it('should add document successfully', () => {
      const document = workspace.addDocument(documentPath, documentTitle, position, dimensions);
    ...
    it('should prevent duplicate documents with same path', () => {
      ...
      expect(() => {
        workspace.addDocument(documentPath, 'Different Title', position, dimensions);
    ...
    it('should maintain single active document invariant', () => {
      const doc1 = workspace.addDocument('/path/doc1.pdf', 'Doc 1', position, dimensions);
      const doc2 = workspace.addDocument('/path/doc2.pdf', 'Doc 2', position, dimensions);
    ...
    it('should check if document is open by path', () => {
      ...
      const isOpen = documents.some(doc => doc.getFilePath() === documentPath);
      const isOtherOpen = documents.some(doc => doc.getFilePath() === '/path/to/other.pdf');
  ...
  describe('Document Operations', () => {
    ...
    it('should update last modified timestamp on document operations', () => {
      ...
      setTimeout(() => {
        workspace.moveDocument(document.getId(), Position.fromCoordinates(150, 150));
  ...
  describe('Layout Mode Management', () => {
    beforeEach(() => {
      ...
      workspace.addDocument('/path/doc1.pdf', 'Doc 1', Position.fromCoordinates(0, 0), Dimensions.fromValues(300, 400));
      workspace.addDocument('/path/doc2.pdf', 'Doc 2', Position.fromCoordinates(100, 100), Dimensions.fromValues(350, 450));
      workspace.addDocument('/path/doc3.pdf', 'Doc 3', Position.fromCoordinates(200, 200), Dimensions.fromValues(320, 380));
    });
    ...
    it('should rearrange documents when switching to grid mode', () => {
      const initialPositions = workspace.getAllDocuments().map(doc => doc.getPosition());

      workspace.switchLayoutMode(LayoutMode.fromString(LayoutModeType.GRID));

      const gridPositions = workspace.getAllDocuments().map(doc => doc.getPosition());
      ...
      const sortedPositions = gridPositions.sort((a, b) => a.getX() - b.getX() || a.getY() - b.getY());
      expect(sortedPositions[0].getX()).toBeLessThanOrEqual(sortedPositions[1].getX());
    });
    ...
    it('should preserve positions when switching to freeform mode', () => {
      ...
      // Switch to freeform
      workspace.switchLayoutMode(LayoutMode.fromString(LayoutModeType.FREEFORM));
      const freeformPositions = workspace.getAllDocuments().map(doc => doc.getPosition());
  ...
  describe('Workspace State Validation', () => {
    ...
    it('should maintain workspace invariants', () => {
      ...
      for (let i = 0; i < 10; i++) {
        workspace.addDocument(`/path/doc${i}.pdf`, `Doc ${i}`, Position.fromCoordinates(i * 10, i * 10), Dimensions.fromValues(300, 400));
      }
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle workspace size changes', () => {
      workspace.addDocument('/path/doc1.pdf', 'Doc 1', Position.fromCoordinates(800, 600), Dimensions.fromValues(300, 400));
      ...
      expect(document.getPosition().getX() + document.getDimensions().getWidth()).toBeLessThanOrEqual(newSize.getWidth());
    });

    it('should handle concurrent document operations', () => {
      const doc1 = workspace.addDocument('/path/doc1.pdf', 'Doc 1', Position.fromCoordinates(0, 0), Dimensions.fromValues(300, 400));
      const doc2 = workspace.addDocument('/path/doc2.pdf', 'Doc 2', Position.fromCoordinates(100, 100), Dimensions.fromValues(350, 450));
