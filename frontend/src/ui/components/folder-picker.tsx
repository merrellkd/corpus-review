...
import { FolderPath } from "../../features/project";
...
export const FolderPicker: React.FC<FolderPickerProps> = ({
  ...
  const containerClasses = ["folder-picker", className]
    .filter(Boolean)
    .join(" ");
        ...
        <div className="mt-2 p-3 bg-gray-50 border border-gray-200 rounded-md text-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <div className="mr-2">
                {folderInfo.exists ? (
                  <span className="text-green-600" title="Folder exists">
                    ✓
                  </span>
                ) : (
                  <span className="text-red-600" title="Folder not found">
                    ⚠️
                  </span>
                )}
              </div>
              <div>
                ...
                <div
                  className="text-gray-600 text-xs truncate max-w-xs"
          ...
          {(folderInfo.itemCount !== undefined ||
            ...
            <div className="mt-2 text-xs text-gray-500 space-x-4">
              ...
              {folderInfo.lastModified && (
                <span>
                  Modified {folderInfo.lastModified.toLocaleDateString()}
                </span>
              )}
            </div>
          )}
 ...
 */
export const SimpleFolderPicker: React.FC<
  Omit<FolderPickerProps, "showFolderInfo">
> = (props) => {
 ...
 */
export const ProjectFolderPicker: React.FC<
  Omit<FolderPickerProps, "validate">
> = (props) => {
  const validateProjectFolder = useCallback(
  ...
  return (
    <FolderPicker
      ...
      helpText={
        props.helpText ||
        "Select a folder to use as the project source directory"
      }
    />
  );
};
