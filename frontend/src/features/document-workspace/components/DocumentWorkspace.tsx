...
import { LayoutModeType } from "@/features/workspace/domain/value-objects/layout-mode";
import {
  Position,
  Dimensions,
...
import { WorkspaceCommandBar } from "@/features/workspace/ui/components/WorkspaceCommandBar";
import DocumentCaddy from "@/features/workspace/ui/components/DocumentCaddy";
...
export const DocumentWorkspace: React.FC = () => {
  ...
  const handleDocumentMove = useCallback(
    (documentId: string, position: Position) => {
  ...
  const handleDocumentResize = useCallback(
    (documentId: string, dimensions: Dimensions) => {
      if (!currentWorkspace) return;
  ...
  const handleDocumentTitleChange = useCallback(
    (documentId: string, newTitle: string) => {
      console.log(
        "Document title change not implemented:",
        documentId,
        newTitle
      );
    },
      ...
      <div className="flex flex-col items-center justify-center h-full text-gray-500">
        <div className="text-center max-w-md">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Your workspace is ready. Documents can be added through other parts
            of the application.
          </p>
        </div>
      </div>
      ...
      for (const entry of entries) {
        ...
        if (width > 0 && height > 0) {
          ...
          if (lastResize) {
            ...
            const heightChanged =
              Math.abs(lastResize.height - height) > tolerance;
          ...
          resizeTimeoutRef.current = setTimeout(() => {
            try {
              ...
              console.log(
                "Update workspace dimensions not implemented:",
                newDimensions
              );
            } catch (error) {
              console.warn(
                "Invalid workspace dimensions during resize:",
                error
              );
            }
          }, 100); // 100ms debounce
  ...
  if (hasError) {
    return (
      <div
        className="h-full bg-gray-50 relative overflow-hidden flex flex-col"
      ...
      <div className="flex items-center space-x-2 text-sm text-gray-600">
        ...
        <span
          className={`px-2 py-1 rounded text-xs font-medium ${
            currentWorkspace.layoutMode === LayoutModeType.STACKED
              ? "bg-blue-100 text-blue-800"
              : currentWorkspace.layoutMode === LayoutModeType.GRID
      ...
      <div className="absolute bottom-4 left-4 z-10 bg-white bg-opacity-90 backdrop-blur-sm rounded-lg px-3 py-2 shadow-sm border border-gray-200">
        <div className="flex items-center space-x-4 text-xs text-gray-600">
          ...
          <span>
            ...
            {currentWorkspace.workspaceDimensions.width}×
            {currentWorkspace.workspaceDimensions.height}
          </span>
        </div>
      </div>
    );
  ...
  return (
    <div
      className="multi-document-workspace flex flex-col h-full"
      data-testid="document-workspace-panel"
    >
      {/* Command Bar */}
      <WorkspaceCommandBar
      ...
      <div
        ...
        tabIndex={-1}
      >
        <div className={getWorkspaceClasses()} style={documentContainerStyle}>
          {documents.length === 0 ? (
            renderEmptyState
          ) : (
            <>
