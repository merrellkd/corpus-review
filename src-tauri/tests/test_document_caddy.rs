...
async fn test_create_document_caddy_with_valid_file_path() {
    ...
    assert!(result.is_ok(), "create_document_caddy should return Ok for valid file path");

    // Parse JSON response to verify structure
    let response_json: serde_json::Value = serde_json::from_str(&result.unwrap())
        .expect("Response should be valid JSON");

    assert!(response_json.get("caddy").is_some(), "Response should contain 'caddy' field");
    assert!(response_json["caddy"].is_object(), "caddy should be an object");
}
...
async fn test_create_document_caddy_with_nonexistent_file() {
    ...
    assert!(result.is_ok(), "create_document_caddy should return Ok even for nonexistent files");

    let response_json: serde_json::Value = serde_json::from_str(&result.unwrap())
        .expect("Response should be valid JSON");

    assert!(response_json.get("error").is_some(), "Response should contain 'error' field for nonexistent file");
}
...
async fn test_create_document_caddy_with_invalid_workspace_id() {
    ...
    assert!(result.is_ok(), "create_document_caddy should return Ok even for invalid workspace ID");

    let response_json: serde_json::Value = serde_json::from_str(&result.unwrap())
        .expect("Response should be valid JSON");

    // Should return error for invalid workspace ID
    assert!(response_json.get("error").is_some(), "Response should contain 'error' field for invalid workspace ID");
}
...
async fn test_create_document_caddy_response_structure() {
    ...
    if response_json.get("error").is_none() {
        ...
        assert!(response_json.get("caddy").is_some(), "Response should have 'caddy' field");
        let caddy = &response_json["caddy"];

        // Verify DocumentCaddyDto structure
        assert!(caddy.get("id").is_some(), "caddy should have 'id' field");
        assert!(caddy.get("filePath").is_some(), "caddy should have 'filePath' field");
        assert!(caddy.get("title").is_some(), "caddy should have 'title' field");
        assert!(caddy.get("isActive").is_some(), "caddy should have 'isActive' field");
        assert!(caddy.get("position").is_some(), "caddy should have 'position' field");
        assert!(caddy.get("dimensions").is_some(), "caddy should have 'dimensions' field");
        assert!(caddy.get("scrollPosition").is_some(), "caddy should have 'scrollPosition' field");

        // Verify position structure (CaddyPositionDto)
        let position = &caddy["position"];
        assert!(position.get("x").is_some(), "position should have 'x' field");
        assert!(position.get("y").is_some(), "position should have 'y' field");
        assert!(position.get("zIndex").is_some(), "position should have 'zIndex' field");

        // Verify dimensions structure (CaddyDimensionsDto)
        let dimensions = &caddy["dimensions"];
        assert!(dimensions.get("width").is_some(), "dimensions should have 'width' field");
        assert!(dimensions.get("height").is_some(), "dimensions should have 'height' field");
        assert!(dimensions.get("minWidth").is_some(), "dimensions should have 'minWidth' field");
        assert!(dimensions.get("minHeight").is_some(), "dimensions should have 'minHeight' field");

        // Verify data types
        assert!(caddy["id"].is_string(), "caddy.id should be string");
        assert!(caddy["filePath"].is_string(), "caddy.filePath should be string");
        assert!(caddy["title"].is_string(), "caddy.title should be string");
        assert!(caddy["isActive"].is_boolean(), "caddy.isActive should be boolean");
        assert!(caddy["scrollPosition"].is_number(), "caddy.scrollPosition should be number");

        assert!(position["x"].is_number(), "position.x should be number");
        assert!(position["y"].is_number(), "position.y should be number");
        assert!(position["zIndex"].is_number(), "position.zIndex should be number");

        assert!(dimensions["width"].is_number(), "dimensions.width should be number");
        assert!(dimensions["height"].is_number(), "dimensions.height should be number");
        assert!(dimensions["minWidth"].is_number(), "dimensions.minWidth should be number");
        assert!(dimensions["minHeight"].is_number(), "dimensions.minHeight should be number");
    }
}
...
async fn test_create_document_caddy_with_various_file_types() {
    ...
    for file_path in file_paths {
        ...
        assert!(result.is_ok(), "create_document_caddy should work for file type: {}", file_path);
        ...
        assert!(
            ...
            "Response should contain either 'caddy' or 'error' field for file: {}", file_path
        );
    }
}
...
async fn test_create_document_caddy_caddy_id_format() {
    ...
    if response_json.get("error").is_none() {
        ...
        assert!(caddy_id.starts_with("doc_"), "caddy ID should start with 'doc_' prefix");
        assert!(caddy_id.len() > 4, "caddy ID should be longer than just the prefix");
...
async fn test_create_document_caddy_title_extraction() {
    ...
    if response_json.get("error").is_none() {
        ...
        assert_eq!(title, "My Important Document", "Title should be extracted from filename");
        assert!(!title.contains("/"), "Title should not contain path separators");
        assert!(!title.contains(".txt"), "Title should not contain file extension");
    }
}