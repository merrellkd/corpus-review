...
async fn test_list_folder_contents_with_valid_folder() {
    ...
    assert!(
        result.is_ok(),
        "list_folder_contents should return Ok for valid folder path"
    );
    ...
    assert!(
        response_json.get("items").is_some(),
        "Response should contain 'items' field"
    );
    assert!(
        response_json["items"].is_array(),
...
async fn test_list_folder_contents_with_inaccessible_folder() {
    ...
    assert!(
        result.is_ok(),
        "list_folder_contents should return Ok even for inaccessible folders"
    );
    ...
    assert!(
        ...
        "Response should contain 'error' field for inaccessible folder"
    );
    assert!(
        response_json["items"].is_array(),
        "Items field should still be an array (empty)"
    );
    assert_eq!(
        response_json["items"].as_array().unwrap().len(),
        0,
        "Items array should be empty for inaccessible folder"
    );
}
...
async fn test_list_folder_contents_with_empty_folder() {
    ...
    assert!(
        result.is_ok(),
        "list_folder_contents should return Ok for empty but accessible folder"
    );
    ...
    assert!(
        ...
        "Should not have error for accessible empty folder"
    );
}
...
async fn test_list_folder_contents_response_structure() {
    ...
    if let Some(items) = response_json["items"].as_array() {
        if !items.is_empty() {
            ...
            assert!(
                first_item.get("path").is_some(),
                "Item should have 'path' field"
            );
            assert!(
                first_item.get("name").is_some(),
                "Item should have 'name' field"
            );
            assert!(
                first_item.get("type").is_some(),
                "Item should have 'type' field"
            );
            assert!(
                first_item.get("lastModified").is_some(),
                "Item should have 'lastModified' field"
            );
            assert!(
                first_item.get("isAccessible").is_some(),
                "Item should have 'isAccessible' field"
            );
...
async fn test_list_folder_contents_with_special_characters() {
    ...
    assert!(
        result.is_ok(),
        "list_folder_contents should handle special characters in folder path"
    );
